...
static vk::raii::Instance createVulkanInstance(const vk::raii::Context &context)
{
    ...
    auto instanceResult = context.createInstance(instanceInfo);
    if (!instanceResult && !validationLayers.empty()) {
        // try again without the validation layer
        validationLayers.clear();
        instanceInfo.setPEnabledLayerNames(validationLayers);
        auto instanceResult2 = context.createInstance(instanceInfo);
        if (instanceResult2) {
            qCWarning(KWIN_CORE, "Vulkan validation layer is not installed");
            return std::move(instanceResult2.value());
        }
    }
...
static std::unique_ptr<VulkanDevice> openVulkanDevice(const vk::raii::Instance &instance, DrmDevice *drm)
{
    ...
    if (!physicalDevicesResult) {
        qCWarning(KWIN_VULKAN) << "querying vulkan devices failed:" << vk::to_string(physicalDevicesResult.error());
        return nullptr;
    }
    const auto &physicalDevices = physicalDevicesResult.value();
#if HAVE_LIBDRM_FAUX
    ...
    for (const vk::raii::PhysicalDevice &physicalDevice : physicalDevices) {
        ...
        for (auto it = missingExtensions.begin(); it != missingExtensions.end();) {
            const auto isPresent = std::ranges::any_of(extensionProps, [required = *it](const auto &ext) {
        ...
        auto logicalDeviceResult = physicalDevice.createDevice(deviceInfo);
        if (!logicalDeviceResult) {
            qCWarning(KWIN_VULKAN, "vkCreateDevice for %s failed: %s", deviceName, vk::to_string(vk::Result(logicalDeviceResult.error())).c_str());
            continue;
        }

        auto ret = std::make_unique<VulkanDevice>(
            physicalDevice,
            std::move(logicalDeviceResult.value()),
