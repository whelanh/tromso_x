...
QImage VulkanTexture::download() const
{
    ...
    auto stagingBufferResult = m_device->logicalDevice().createBuffer(bufferInfo);
    if (!stagingBufferResult) {
        return {};
    }
    auto &stagingBuffer = stagingBufferResult.value();
    ...
    void *dataPtr = stagingMemory.mapMemory(0, bufferSize);
    if (!dataPtr) {
        return {};
    }
...
bool VulkanTexture::update(const QImage &img, const Region &region, const QPoint &offset)
{
    ...
    auto &stagingBuffer = stagingBufferResult.value();
    stagingBuffer.bindMemory(stagingMemory, 0);
    void *dataPtr = stagingMemory.mapMemory(0, vk::DeviceSize(img.sizeInBytes()));
    if (!dataPtr) {
        return false;
    }
...
std::unique_ptr<VulkanTexture> VulkanTexture::allocate(VulkanDevice *device, vk::Format format, const QSize &size, vk::ImageUsageFlags usage)
{
    ...
    auto imageResult = device->logicalDevice().createImage(info);
    if (!imageResult) {
        qCWarning(KWIN_VULKAN) << "creating image failed!" << vk::to_string(imageResult.error());
        return nullptr;
    }
