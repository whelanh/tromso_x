...
std::optional<RenderTimeSpan> VulkanRenderTimeQuery::query()
{
    ...
    if (!m_result) {
        auto timestampsResult = m_pool.getResults<uint64_t>(0, 2, 2 * sizeof(uint64_t), sizeof(uint64_t), vk::QueryResultFlagBits::e64 | vk::QueryResultFlagBits::eWait);
        if (!timestampsResult) {
            reset();
            return std::nullopt;
        }
        const auto &timestamps = timestampsResult.value();
...
std::unique_ptr<VulkanRenderTimeQuery> VulkanRenderTimeQuery::begin(VulkanDevice *device, vk::raii::CommandBuffer &buffer, uint32_t queueFamily)
{
    ...
    auto queryResult = device->logicalDevice().createQueryPool(vk::QueryPoolCreateInfo{
