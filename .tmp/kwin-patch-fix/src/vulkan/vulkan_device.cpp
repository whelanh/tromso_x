...
void VulkanDevice::createCommandPool()
{
    ...
    auto cmdPoolResult = m_logical.createCommandPool(vk::CommandPoolCreateInfo{
        vk::CommandPoolCreateFlagBits::eResetCommandBuffer,
        m_queueFamilyIndex,
    });
    if (!cmdPoolResult) {
        qCCritical(KWIN_VULKAN) << "creating a command pool failed:" << vk::to_string(cmdPoolResult.error());
        return;
    }
    m_commandPool = std::move(cmdPoolResult.value());
}
...
std::shared_ptr<VulkanTexture> VulkanDevice::importDmabuf(const DmaBufAttributes *attributes, VkImageUsageFlags usage)
{
    ...
    auto imageResult = m_logical.createImage(imageInfo);
    if (!imageResult) {
        qCWarning(KWIN_VULKAN) << "creating vulkan image failed!" << vk::to_string(imageResult.error());
        return nullptr;
    }
    ...
    for (uint32_t i = 0; i < memoryCount; i++) {
        const auto memoryFdResult = m_logical.getMemoryFdPropertiesKHR(vk::ExternalMemoryHandleTypeFlagBits::eDmaBufEXT, duplicatedFds[i].get());
        if (!memoryFdResult) {
            qCWarning(KWIN_VULKAN) << "failed to get memory fd properties!" << vk::to_string(memoryFdResult.error());
            return nullptr;
        }
        const auto &memoryFdProperties = memoryFdResult.value();
        ...
        auto allocateResult = m_logical.allocateMemory(memoryInfo);
        if (!allocateResult) {
            qCWarning(KWIN_VULKAN, "'Allocating' memory for dmabuf failed: %s", vk::to_string(allocateResult.error()).c_str());
            return nullptr;
        }
        auto &memory = allocateResult.value();
...
vk::raii::CommandBuffer VulkanDevice::createCommandBuffer()
{
    ...
    auto buffersResult = m_logical.allocateCommandBuffers(vk::CommandBufferAllocateInfo{
        m_commandPool,
        vk::CommandBufferLevel::ePrimary,
        1,
    });
    if (!buffersResult) {
        qCWarning(KWIN_VULKAN) << "Failed to create a command buffer" << vk::to_string(buffersResult.error());
        return nullptr;
    }
    return std::move(buffersResult.value().front());
}
...
std::optional<vk::raii::Semaphore> VulkanDevice::importSemaphore(FileDescriptor &&syncFd) const
{
    ...
    auto semaphoreResult = m_logical.createSemaphore(semaphoreInfo);
    if (!semaphoreResult) {
        return std::nullopt;
    }
    ...
    vk::Result result = m_logical.importSemaphoreFdKHR(importInfo);
...
std::optional<FileDescriptor> VulkanDevice::submit(vk::raii::CommandBuffer &&buffer, FileDescriptor &&syncFd)
{
    ...
    auto fenceResult = m_logical.createFence(vk::FenceCreateInfo{
        vk::FenceCreateFlags{},
        &exportInfo,
    });
    if (!fenceResult) {
    ...
    const auto fdResult = m_logical.getFenceFdKHR(vk::FenceGetFdInfoKHR{
        fence,
        vk::ExternalFenceHandleTypeFlagBits::eSyncFd,
    });
    if (!fdResult) {
    ...
    FileDescriptor ret{fdResult.value()};
...
vk::raii::DeviceMemory VulkanDevice::allocateMemory(const vk::ImageCreateInfo &imageInfo, vk::MemoryPropertyFlags memoryProperties)
{
    ...
    if (const auto typeIndex = findMemoryType(requirements.memoryRequirements.memoryTypeBits, memoryProperties)) {
        auto allocateResult = m_logical.allocateMemory(vk::MemoryAllocateInfo{
            requirements.memoryRequirements.size,
            *typeIndex,
        });
        if (allocateResult) {
            return std::move(allocateResult.value());
        } else {
            qCWarning(KWIN_VULKAN) << "Allocating memory for an image failed:" << vk::to_string(allocateResult.error());
            return nullptr;
        }
    } else {
...
vk::raii::DeviceMemory VulkanDevice::allocateMemory(const vk::BufferCreateInfo &bufferInfo, vk::MemoryPropertyFlags memoryProperties)
{
    ...
    if (const auto typeIndex = findMemoryType(requirements.memoryRequirements.memoryTypeBits, memoryProperties)) {
        ...
        if (allocateResult) {
            ...
            qCWarning(KWIN_VULKAN) << "Allocating memory for a buffer failed:" << vk::to_string(allocateResult.error());
            return nullptr;
        }
    } else {
